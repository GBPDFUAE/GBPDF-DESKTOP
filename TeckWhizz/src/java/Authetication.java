 ...
 * @author Win7
 */
@WebServlet(urlPatterns = {"/Authetication"})
public class Authetication extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
           ...
           if(mres){
               boolean res = DB_SERVICE.DOA_Service.checkUserLogin(stud_mobno, pwd);
            if (res) {
...
// dis.forward(request, response);
              ...
              // String sid=   DB_SERVICE.DOA_Service.getStudentLastMobileNumberbyid();
                ...
                request.setAttribute("cmsg", "INVALID PASSWORD");
                RequestDispatcher dis = request.getRequestDispatcher("Login.jsp");
                dis.forward(request, response);
            }
           }else
           {
              request.setAttribute("cmsg", "* Mobile Number Not Registered with WISHTOPIA");
                RequestDispatcher dis = request.getRequestDispatcher("Login.jsp");
                dis.forward(request, response);  
           }
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Authetication</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Authetication at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        } finally {
            out.close();
